        ...
        for j in range(image.shape[1]):
            if str(image[i, j]) not in colors.keys():
                if (image[i, j, 0] == image[i, j, 1] == image[i, j, 2] >= int(random.uniform(150, 200))):
                    ...
                    newColor[0] = trim(newColor[0] + int(random.uniform(-100, 100)))
                    newColor[1] = trim(newColor[1] + int(random.uniform(-100, 100)))
                    newColor[2] = trim(newColor[2] + int(random.uniform(-100, 100)))
...
def collage(images):
    ...
    for i in range(0, 3):
        for j in range(0, 3):
            out[i * images[counter].shape[0] : (i + 1) * images[counter].shape[0], j * images[counter].shape[1]: (j + 1) * images[counter].shape[1]] = images[counter]
            counter += 1
    return out
...
input = "input.png" #input the name of your start image file here
...
cv2.imwrite('output.png', collage(imageList))
